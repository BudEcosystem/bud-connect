...
def compile_upsert(table: Table, row: dict, conflict_target: list, set_: dict | None = None) -> str:
    ...
    clause = conflict_set_clause(stmt, row if set_ is None else set_(stmt) if callable(set_) else set_, conflict_target)
...
def test_every_upsert_routes_its_set_clause_through_the_helper(path_line_call):
    ...
    assert isinstance(set_kw.value, ast.Call) and ast.unparse(set_kw.value.func).endswith(
        "conflict_set_clause"
    ), (
